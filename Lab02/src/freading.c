...
stream_read_bytes(FILE *fp, char *buf, ssize_t len, size_t incr)
{
  ...
  while(total<len) {
    if(len-total < (ssize_t)incr) { cur_incr = len-total; } // If the remaining bytes is less than i>

    ssize_t num_read = fread(&buf[total], 1, cur_incr, fp);
...
_main(int argc, char **argv)
{
  ...
  size_t num_items = fsize / blk;
  ssize_t num_bytes_read = stream_read_bytes(stream, buffer, num_items, blk);
...
stream_read_bytes(FILE *fp, char *buf, ssize_t len, size_t incr)
{
  size_t num_read = fread(buf, incr, len, fp);
  return num_read;
}
